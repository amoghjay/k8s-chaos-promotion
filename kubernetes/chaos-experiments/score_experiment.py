#!/usr/bin/env python3
"""Chaos experiment scorer: queries Prometheus over an experiment's window and
prints a PASS/FAIL scorecard. Exit 0 = all checks pass, 1 = any fail.

Runs inside the chaos-gate Job (the exit code becomes the promotion verdict) or
manually against a port-forwarded Prometheus via --prom / $PROM_URL.

Thresholds are degraded-mode bars, looser than steady-state SLOs — the gate
fails on collapse, not on bumps.
"""
import argparse
import json
import math
...
# In-cluster default; override with --prom or $PROM_URL.
...
def scalar(vec):
    # Counter exprs are wrapped in sum()/max(), so there's at most one series;
    # empty (metric never fired) counts as 0.
    if not vec:
        return 0.0
    try:
...
# Vacuous-pass guard: if loadgen never ran, every SLO check passes trivially
# against no data. Excludes /livez — liveness probes are a constant ~35
# reqs/window noise floor that would defeat the guard. Attached to postgres +
# redis only: a signer outage legitimately drops user traffic to ~0 (loadgen
# bails before /shorten), so a guard there would false-fail. Floor of 50 has
# wide margin both ways (real runs ~300-430 reqs vs 0 when vacuous).
TRAFFIC_GUARD = {
...
# Per-experiment rules. Each check: name, PromQL expr ({ns}/{w} substituted,
# {w} = duration + settle seconds), agg over the window, op, threshold.
# Zero-tolerance counters use increase() over {w} with threshold 0.5 (less than
# one event).
EXPERIMENTS = {
    "postgres-pod-failure": {
        # PG recovery outlasts the fault: StatefulSet restart + WAL recovery
        # bring dependency_up back only ~35s after the chaos clears, so
        # settle=30 closed the window too early; 60 gives margin.
        "settle": 60,
        "checks": [
            TRAFFIC_GUARD,
            {"name": "0 app restarts (liveness-cascade detector)",
            ...
            # PG-down must surface as a clean 503, not an unhandled 500 — keyed
            # on status="500" exactly so 503s pass.
    ...
    "redis-pod-failure": {
        "settle": 40,   # seconds past the fault to keep scoring (pod recovery tail)
        "checks": [
            TRAFFIC_GUARD,
            {"name": "redirect p95 < 1.2s (cache miss must fast-fail to postgres)",
...
def score(experiment, inject_at, duration):
    ...
    for c in cfg["checks"]:
        ...
        if "increase(" in c["expr"]:
            # Counter checks: one instant query at inject+window, so the
            # increase() lookback covers exactly this experiment's window and
            # events from a prior experiment can't bleed in.
            observed = scalar(query_instant(expr, end))
        else:
            # Gauge/quantile checks: range + aggregate, with a -20s lead so the
            # pre-inject baseline is visible.
